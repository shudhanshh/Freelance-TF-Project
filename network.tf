...
# // Allow cluster access from local
# resource "google_compute_firewall" "k8s-array" {
#   name          = format("%s-k8s-array", var.cluster_name)
#   network       = google_compute_network.network.name
#   direction     = "INGRESS"
#   project       = var.project
#   source_ranges = ["0.0.0.0/0"]
 ...
 provisioner "local-exec" {
     command = <<EOF
         ...
         for i in $(seq 1 20); do
           if gcloud compute ssh ${local.hostname} --project ${var.project} --zone ${var.region}-a --command uptime; gcloud beta compute ssh ${local.hostname} --project ${var.project} --zone ${var.region}-a -- -L8888:127.0.0.1:8888;
  then
             READY="yes"
             break;
           fi
